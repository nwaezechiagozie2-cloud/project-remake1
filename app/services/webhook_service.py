...
from app.domain.interfaces import AgentOrchestrator, ContactsAdapter, CustomerRepository, VendorRepository, WhatsAppAdapter
...
class WebhookService:
    ...
    async def _handle_inbound_message(self, message: ParsedInboundMessage) -> None:
        ...
        if not vendor:
            logger.warning("Unknown vendor for phone_number_id=%s", message.phone_number_id)
            return
        ...
        if await self._check_and_handle_timeout(vendor=vendor, customer=customer, message=message, is_vendor_sender=is_vendor_sender):
            return
        ...
        if decision.customer_text:
            ...
            await self.whatsapp.send_text(vendor=vendor, to=customer_target, body=decision.customer_text)
            await self.customers.record_message(
                vendor_id=vendor["id"],
                customer_id=customer["id"],
                direction="OUTBOUND",
        ...
        if decision.vendor_buttons:
            ...
            vendor_target = vendor.get("whatsapp_number") or message.from_number
            await self.whatsapp.send_text(vendor=vendor, to=vendor_target, body=decision.vendor_text)
            await self.customers.record_message(
                vendor_id=vendor["id"],
                customer_id=customer["id"],
                direction="OUTBOUND",
    ...
    async def _check_and_handle_timeout(
        ...
        *,
        ...
        if status == "WAITING_VENDOR_CHECKOUT_APPROVAL" and age >= timedelta(minutes=self.VENDOR_APPROVAL_TIMEOUT_MINUTES):
            ...
            await self._send_and_record_timeout_messages(vendor=vendor, customer=customer, customer_target=message.from_number, customer_text=customer_text, vendor_text=vendor_text)
            await self.customers.upsert_order_lifecycle_state(
                vendor_id=vendor["id"],
                customer_id=customer["id"],
    ...
    async def _send_and_record_timeout_messages(
        ...
        *,
        ...
        await self.whatsapp.send_text(vendor=vendor, to=customer_target, body=customer_text)
        await self.customers.record_message(
            vendor_id=vendor["id"],
            customer_id=customer["id"],
            direction="OUTBOUND",
        ...
        await self.whatsapp.send_text(vendor=vendor, to=vendor_target, body=vendor_text)
        await self.customers.record_message(
            vendor_id=vendor["id"],
            customer_id=customer["id"],
            direction="OUTBOUND",
